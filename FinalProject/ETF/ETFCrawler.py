...
# filter the ETFs
toSave = [] 
with open(argv_list[1], newline = '', errors = 'ignore') as csvfile:
...
filter_csv = argv_list[3] + '/' + argv_list[1]
with open(filter_csv, 'w') as csvfile:
...
while (start - relativedelta(days = 1)).year >= 2013:
...
for i in range(row_num):
    ...
    ETF_df.to_csv(argv_list[2] + '/' + df.at[i, 'Symbol'] + '.csv')
